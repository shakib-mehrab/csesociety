...
const getClubById = asyncHandler(async (req, res) => {
  const club = await Club.findById(req.params.id)
    ...
    .populate('subCoordinators', 'name email')
...
const updateClub = asyncHandler(async (req, res) => {
  const { name, description, coordinator, subCoordinators, contactEmail, logo } = req.body;
...
const processJoinRequest = asyncHandler(async (req, res) => {
    ...
    if (club.coordinator.toString() !== req.user._id.toString() && req.user.role !== 'super_admin') {
        res.status(403);
